...
<div class="content d-flex flex-column flex-column-fluid">
 ...
 <div class="card">
    <div class="card-body">
    <form action="{{ route('update.return.purchase', $purchase->id) }}" method="post" enctype="multipart/form-data">
                  @csrf
       ...
       <div class="row">
          ...
          <div class="col-md-4 mb-3">
                ...
                <select name="warehouse_id" id="warehouse_id" class="form-control form-select" disabled>
                      ...
                      <option value="{{ $item->id }}" {{ $purchase->warehouse_id == $item->id ? 'selected' : '' }}>{{ $item->name }}</option>
                      @endforeach
                </select>
           ...
           <tbody id="productBody">
            ...
            <tr data-id="{{ $item->id }}">
                <td class="d-flex align-items-center gap-2">
                    ...
                    data-cost="{{ $item->product->net_unit_cost }}"
                ...
                <td>
                    <input type="number" name="products[{{ $item->product->id }}][net_unit_cost]" class="form-control net-cost"
                    style="max-width: 90px" value="{{ $item->net_unit_cost }}" readonly>
                </td>
                <td>
                    <input type="number" name="products[{{ $item->product->id }}][stock]" class="form-control net-cost" value="{{ $item->product->product_qty }}"
                ...
                <td>
                    <input type="number" class="form-control discount-input"
                        name="products[{{ $item->product->id }}][discount]" value="{{ $item->discount }}" min="0" style="width: 100px">
                </td>
                <td class="subtotal">{{ number_format($item->subtotal, 2) }}</td>
                <input type="hidden" class="form-control discount-input"
                        name="products[{{ $item->product->id }}][subtotal]" value="{{ $item->subtotal }}" min="0" style="width:100px">

                <td><button class="btn btn-danger btn-sm remove-item" data-id="{{ $item->id }}"><span class="mdi mdi-delete-circle mdi-18px"></span></button></td>
            </tr>
            @endforeach
           </tbody>
        </table>
    ...
    <div class="card">
             ...
             <table class="table border">
                <tbody>
                   <tr>
                      ...
                      <td class="py-3" id="displayDiscount">TK {{ $purchase->discount }}</td>
                   </tr>
                   <tr>
                      <td class="py-3">Shipping</td>
                      <td class="py-3" id="shippingDisplay">TK {{ $purchase->shipping }}</td>
                   </tr>
                   <tr>
                      <td class="py-3 text-primary">Grand Total</td>
                      <td class="py-3 text-primary" id="grandTotal">TK  {{ $purchase->grand_total }}</td>
      ...
      <div class="row">
         <div class="col-md-4">
            ...
            <input type="number" id="inputDiscount" name="discount" class="form-control" value="{{ $purchase->discount }}">
         </div>
         <div class="col-md-4">
            <label class="form-label">Shipping: </label>
            <input type="number" id="inputShipping" name="shipping" class="form-control" value="{{ $purchase->shipping }}">
         </div>
         <div class="col-md-4">
            <div class="form-group w-100">
               ...
               <select name="status" id="status" class="form-control form-select">
                  ...
                  <option value="returned" {{ $purchase->status == "returned" ? 'selected' : '' }}>Returned</option>
      ...
      <div class="col-md-12 mt-2">
         ...
         <textarea class="form-control" name="note" rows="3" placeholder="Enter Notes">{{ $purchase->note }}</textarea>
      </div>
   </div>
</div>
</div>
